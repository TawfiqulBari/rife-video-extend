...
from config import (
    ...
    get_replicate_api_token, check_continuation_dependencies
)
...
def run_continuation_cli(args):
    ...
    # Get API token
    api_token = args.api_token or get_replicate_api_token()

    if not api_token:
        print("ERROR: Replicate API token not configured")
        print("Set via --api-token or REPLICATE_API_TOKEN environment variable")
        sys.exit(1)
