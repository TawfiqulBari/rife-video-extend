...
class RIFEExtenderApp(ctk.CTk):
    ...
    def _create_continuation_controls(self, parent):
        ...
        # API Token
        api_token_label = ctk.CTkLabel(api_frame, text="Replicate API Token:", font=ctk.CTkFont(size=12))
        api_token_label.grid(row=0, column=0, sticky="w", pady=(0, 5))

        self.api_token_entry = ctk.CTkEntry(api_frame, show="*", width=280)
        self.api_token_entry.grid(row=0, column=1, padx=(10, 0), pady=(0, 5))

        # Save credentials checkbox
        self.save_creds_check = ctk.CTkCheckBox(
            api_frame,
            text="Save token locally",
    ...
    def _start_continuation_processing(self):
        ...
        if not api_token:
            self.status_label.configure(text="Error: Replicate API token required", text_color="red")
            return

        # Save credentials if requested
        if self.save_credentials.get():
            save_replicate_config(api_token)
        ...
        thread = threading.Thread(
            ...
            args=(output_path, api_token, prompt, duration, concat),
            daemon=True
        )
        thread.start()
